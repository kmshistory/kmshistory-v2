...
export default function PrivacyCollection() {
  const [title, setTitle] = useState('개인정보 수집 및 이용동의');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const response = await apiClient.get('/settings/collection');
        if (response.status === 200) {
          setTitle(response.data.title || '개인정보 수집 및 이용동의');
          setContent(response.data.content || '');
        }
      } catch (err) {
        console.error('개인정보 수집 및 이용동의 조회 실패:', err);
        setError('개인정보 수집 및 이용동의를 불러올 수 없습니다.');
      } finally {
        setLoading(false);
      }
    };
